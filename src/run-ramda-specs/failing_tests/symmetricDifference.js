...
describe('symmetricDifference', function () {
  ...
  it('returns empty arrays when receiving twice the same array', function () {
    fc.assert(
      fc.property(fc.clone(compatibleREquals, 2), function (arrays) {
        var A1 = arrays[0]
        var A2 = arrays[1]
  ...
  it('returns empty arrays when receiving an array and a permutation of it', function () {
    fc.assert(
      fc.property(
        fc.clone(compatibleREquals, 2).chain(function (arrays) {
          return fc.tuple(
            fc.constant(arrays[0]),
            fc.shuffledSubarray(
              arrays[1],
              arrays[1].length,
              arrays[1].length
    ...
    fc.assert(
      fc.property(
        ...
        function (A1, A2, B, C1, C2) {
          ...
          var difference = R.symmetricDifference(M, N)
          eq(
            R.symmetricDifference(difference, MN).length,
            MN.length - difference.length
          )
        }
      )
    )
