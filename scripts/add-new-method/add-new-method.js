...
async function createMethodFile(methodName){
  const methodPath = resolve(__dirname, `../../../rambda/source/${ methodName }.js`)
...
async function createTestFile(methodName, ramdaSpecs){
  const testPath = resolve(__dirname, `../../../rambda/source/${ methodName }.spec.js`)
...
async function createTypescriptTestFile(methodName){
  const typescriptTestPath = resolve(__dirname, `../../../rambda/source/${ methodName }-spec.ts`)
...
async function addNewMethod(methodName){
  ...
  const ramdaSpecPath = resolve(__dirname,
    `../../src/run-ramda-specs/ramda/test/${ methodName }.js`)

  const ramdaSpecs = existsSync(resolve(__dirname, ramdaSpecPath)) ?
    (await readFile(ramdaSpecPath)).toString() :
    false

  const descriptionPath = resolve(__dirname, '../../../rambda/files/index.d.ts')
